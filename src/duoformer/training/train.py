...
def set_seed(seed: int) -> None:
    ...
    Args:
        seed: Random seed value. Must be non-negative integer.

    Raises:
        ValidationError: If seed is negative.

    Example:
        >>> set_seed(42)
        >>> # All subsequent random operations will be reproducible
    """
    validate_positive_number(seed, "seed", allow_zero=True)
    random.seed(seed)
    np.random.seed(seed)
...
def parse_args() -> argparse.Namespace:
    ...
    Returns:
        argparse.Namespace: Parsed command line arguments with the following attributes:
            ...
            - backbone: Backbone architecture ('r50', 'r18', etc.)
            - num_layers: Number of multi-scale layers
...
def main() -> None:
    """Main training function for DuoFormer medical image classification.
    ...
    - Training loop with checkpointing and monitoring
    - Model evaluation and saving

    The function supports both configuration file-based and command-line argument
    training with automatic hardware detection and optimization.

    Raises:
        ValidationError: If input parameters are invalid
        DataError: If dataset files are not found or invalid
    ...
    # Validate inputs
    if args.data_dir:
        validate_directory_exists(Path(args.data_dir), "data directory")

    if args.config:
        validate_file_exists(Path(args.config), "config file")

    if args.resume:
        validate_file_exists(Path(args.resume), "checkpoint file")

    if args.epochs:
        validate_positive_number(args.epochs, "epochs")

    if args.batch_size:
        validate_positive_number(args.batch_size, "batch_size")

    if args.learning_rate:
        validate_positive_number(args.learning_rate, "learning_rate")
    ...
    if args.gpu_ids:
        ...
        logger.info(f"🎯 Restricting to GPUs: {args.gpu_ids}")
    ...
    if args.num_workers == 4:  # Default value
        ...
        logger.info(f"📊 Optimal num_workers for this platform: {args.num_workers}")
    ...
    logger.info(
        f"Depth: {args.depth}, Embed Dim: {args.embed_dim}, Heads: {args.num_heads}"
    )
    logger.info(f"Scales: {args.num_layers}, Proj Dim: {args.proj_dim}")
