...
from .multi_vision_transformer import MultiscaleTransformer
from .multiscale_attn import MultiScaleAttention, MultiscaleBlock
...
from .backbone import Backbone, Backbone2

# Use standard torchvision models instead of legacy implementations
import torchvision.models as models
from torchvision.models import ResNet50_Weights, ResNet18_Weights


class DuoFormerModel(nn.Module):
    """DuoFormer model without extra parameters for medical image classification.

    This is the main DuoFormer implementation that combines ResNet backbones with
    multi-scale transformers for general medical image classification tasks.

    The model architecture consists of:
    1. ResNet backbone for feature extraction at multiple scales
    2. Multi-scale transformer blocks for attention across scales
    3. Classification head for final predictions

    Attributes:
        backbone_name: Name of the backbone architecture ('r50', 'r18', 'swav')
        num_classes: Number of output classes
        num_layers: Number of multi-scale layers (2, 3, or 4)
        embed_dim: Embedding dimension for transformers
        num_heads: Number of attention heads
        proj_dim: Projection dimension for multi-scale features

    Example:
        >>> model = DuoFormerModel(
        ...     backbone='r50',
        ...     num_classes=10,
        ...     num_layers=2,
        ...     embed_dim=768,
    ...
    def get_features(self, x):
        ...
        Returns:
            dict: Dictionary containing features at different scales with keys:
                - '0': Features from layer 0 (early layers)
                - '1': Features from layer 1 (middle layers)
                - '2': Features from layer 2 (later layers)
                - '3': Features from layer 3 (final layers)
        """
        features = {}
        ...
        # Handle ResNet feature extraction properly
        if self.backbone == "r50":
            # ResNet-50 Sequential structure: [conv1, bn1, relu, maxpool, layer1, layer2, layer3, layer4]
            layer_indices = {
                "4": "0",
                "5": "1",
                "6": "2",
            ...
            for i, module in enumerate(self.resnet_projector):
                x = module(x)
                if str(i) in layer_indices:
                    features[layer_indices[str(i)]] = x
        else:
            # For other backbones, use the original approach
    ...
    def forward(self, x):
        """Forward pass through the DuoFormer model.

        Args:
            x: Input tensor of shape [batch_size, channels, height, width]
               Expected input size is typically [batch_size, 3, 224, 224]

        Returns:
            torch.Tensor: Classification logits of shape [batch_size, num_classes]

        Raises:
            RuntimeError: If model encounters an error during forward pass

        Example:
            >>> model = MyModel_no_extra_params(num_classes=10)
            >>> input_tensor = torch.randn(4, 3, 224, 224)
            >>> output = model(input_tensor)  # Shape: [4, 10]
        """
        ...
        if not hasattr(self, "_indices_device_moved") and isinstance(x, dict) and x:
